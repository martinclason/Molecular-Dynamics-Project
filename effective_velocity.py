...
def longtitudinalSoundWaveVelocity(atoms_object, calc) :
    ...
    G = shear_modulus(atoms_object, calc)
    rho = density(atoms_object) #g/cm3
    rho = rho * 1E3 #kg/m3
    c_l = ((K + 4/3 * G)/rho)**0.5 #longitudinal sound wave velocity
    return c_l

def transversalSoundWaveVelocity(atoms_object, calc) :
...
def effectiveVelocity(atoms_object, calc) :
    ...
    c_t = transversalSoundWaveVelocity(atoms_object, calc)
    c_l = longtitudinalSoundWaveVelocity(atoms_object, calc)
